...
for i in range(len(model.outputs)):
    # (x2, y2)
    model.outputs[i] += EPS
    x2 = float(model.outputs[i])    # since there is only one input feature
    cost2 = mse.forward(model.outputs, y)    # y2
    model.outputs[i] -= EPS    # reset our outputs

    # (x1, y1)
    model.outputs[i] -= EPS
    x1 = float(model.outputs[i])    # since there is only one input feature
    cost1 = mse.forward(model.outputs, y)    # y1
    model.outputs[i] += EPS    # reset our outputs
    
    slope_yhat.append(float((cost2 - cost1) / (x2 - x1)))
...
slope_w = np.dot(slope_yhat, ((y2 - y1) / (x2 - x1)))
...
slope_b = np.dot(slope_yhat, ((y2 - y1) / (x2 - x1)))
